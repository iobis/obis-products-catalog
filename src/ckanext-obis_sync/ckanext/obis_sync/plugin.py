...
class ObisSyncPlugin(plugins.SingletonPlugin):
...
def sync_nodes():
    ...
    for node in nodes:
        ...
        existing = (
            ...
            .filter_by(name=org_name, type="organization")
            .first()
        )
...
@click.option(
    "--limit", default=None, type=int, help="Limit number of institutions to process"
)
def sync_institutions(limit):
    ...
    def fetch_ocean_expert_data(oe_id):
        try:
            ...
            click.echo(
                f"    Warning: Could not fetch Ocean Expert data for ID {oe_id}: {e}"
            )
            return None
    ...
    try:
        response = requests.get(
            "https://api.obis.org/v3/institute", params={"size": 10000}, timeout=120
        )
        response.raise_for_status()
        data = response.json()
    ...
    for i, inst in enumerate(institutions, 1):
        ...
        try:
            ...
            click.echo(
                f"[{i}/{len(institutions)}] Processing: {inst_name} (OE ID: {oe_id})"
            )
            ...
            if oe_id:
                ...
                if ocean_expert_data:
                    enriched += 1
                    click.echo("  Retrieved Ocean Expert data")
                time.sleep(1)

            oe_institute = (
                ocean_expert_data.get("institute") if ocean_expert_data else None
            )
            ...
            if existing:
                if existing.type == "group":
                    ...
                    click.echo(
                        f"  Skipping: name '{final_slug}' already exists as {existing.type}"
                    )
                    failed += 1
                    continue
            else:
    ...
    if len(institutions) > 0:
        click.echo(
            f"Success rate: {((created + updated) / len(institutions) * 100):.1f}%"
        )
